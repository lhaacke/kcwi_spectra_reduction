...
def fit_vel_sigma(spectrum, save_as, z, shift_spec=True, fit=False, bootstrap=False, smoothed_spec='', fit_z=False):
    '''
    fit spectra from a muse cube
    ...
    lamRange1 = np.array([h1['WAVELMIN']*10, h1['WAVELMAX']*10])
    # fwhm_gal = np.average(lamRange1/3000)
    fwhm_gal = 2.75

    print('fwhm_gal:{}'.format(fwhm_gal))
    ...
    # check for infinite pixels
    c_inf = np.isfinite(galaxy)
    galaxy[~c_inf] = np.nanmedian(galaxy)
    ...
    miles = glob.glob(ppxf_dir + '/miles_models/*.fits')
    ...
    if fit:
        ...
        res = np.recarray(shape = (len(degree)*len(mdegree) + 1), # array to store the result for each degree combination in
        ...
        pp = ppxf(templates, galaxy, noise, velscale, start, goodpixels=goodPixels,
                  plot=True, moments=2, degree=10, mdegree=4,
                  lam=np.exp(ln_lam1), lam_temp=np.exp(ln_lam2),
                  velscale_ratio=velscale_ratio)
        ...
        sn = np.nanmedian(2*np.sqrt((pp.bestfit-pp.apoly)/(pp.galaxy - pp.bestfit))**2)
    ...
    if fit:
        ...
        plt.close()
        return vtot, redshift_best, errors, redshift_err, sn
    else:
        plot_result(pp, save_as, redshift_best, lamRange1, smoothed_spec=smoothed_spec)
        # plt.savefig(save_as)
        plt.close()
...
def plot_result(pp, save_as, z_best, lamRange1, spec=True, shift_z=True, degree=10, smoothed_spec=''):
    ...
    if spec:
        ...
        bestfit = pp.bestfit
        if pp.gas_any:
            stars_bestfit = bestfit - pp.gas_bestfit
